...
*   **Transiciones Cubic-Bezier:** El panel del editor se desliza y desvanece de forma ultrasuave a ancho 0 y opacidad 0 usando una curva de aceleración premium (`cubic-bezier(0.4, 0, 0.2, 1)`) en 300 ms, expandiendo de forma reactiva la vista previa de lectura.
...
*   **Aislamiento de Procesos:** Cumpliendo con **OWASP A01**, separamos el Renderer Process del Main Process. La interfaz no tiene acceso directo a las APIs nativas de Node.js, interactuando únicamente mediante un puente seguro IPC (`src/preload.js`).
*   **Pipeline de Sanitización de HTML (OWASP A03 / XSS):** El parseo de Markdown a HTML se realiza de forma aislada en el Main Process, aplicando una función de sanitización personalizada que purga etiquetas `<script>`, neutraliza pseudoprotocolos `javascript:` y elimina handlers de eventos inline (`onload`, `onerror`).
...
### 💎 Características Markdown Extendidas Premium
...
Para compilar la aplicación y generar el instalador nativo ejecutable ejecutable (`.exe`) de Windows de forma local:
```bash
npm run dist
```
...
2.  Instala las dependencias y compila el instalador nativo portable de la app mediante `electron-builder`.
